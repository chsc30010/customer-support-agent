...
- **A decision journal, and a knowledge-gap report built on it.** One JSONL line per turn, redacted, off unless switched on. The report groups the contacts the agent could not answer and ranks them by frequency, separating a missing article from a missing phrase -- two failures that look identical from the outside and need opposite fixes. This is the flywheel: the agent tells the content team what to write next, from real customers rather than from guesses.
- **Offline shadow replay.** Recorded turns are re-run through the model path and diffed against what was served. Running both paths live would double the latency of a phone call and make the customer pay for the experiment; a customer who has already hung up pays nothing.
...
**Later:** Multi-turn evaluation, because the golden set is single-turn while the agent has behaviour specifically for later turns and none of it is scored. Business-hours awareness on the handoff -- the help centre says agents work 8am to 8pm, and the agent currently promises to put a caller through at any hour. A tool layer for order lookup and refunds under a policy cap, which needs a permission model and an audit log before it needs any code. A real handoff integration that opens a ticket carrying the `Escalation` object rather than only transferring the call. Redis-backed conversation state for more than one worker. Embeddings for gap clustering, once there is enough traffic for lexical grouping to be the bottleneck.
